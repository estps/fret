...
-- storage.lua nodes announce themselves; display assigns each one specific
-- parts to download so no two nodes ever fetch the same part.
local nodes, inflight = {}, {}
local COORD = { active = false, movie = nil, lastPart = 0, head = 0 }
local CHUNK_EST = 1000000
local SPACE_FLOOR = 4500000
local lastCoordTick = 0

local function partPath(movie, i)
    local leaf = movie .. ".ccm." .. i
...
local function coordTick()
    local now = os.clock()
    if now - lastCoordTick < 0.25 then return end
    lastCoordTick = now

    for id, n in pairs(nodes) do
        if now - n.seen > 12 then nodes[id] = nil end
    end
    for p, a in pairs(inflight) do
        if now - a.t > 20 or not nodes[a.node] then inflight[p] = nil end
    end
    if not COORD.active then return end
    if totalFree() < SPACE_FLOOR then return end

    local idle = {}
    for id, n in pairs(nodes) do
        if not n.busy and n.free > CHUNK_EST + 500000 then
            idle[#idle + 1] = id
        end
    end
    if #idle == 0 then return end

    local scanFrom = math.max(0, COORD.head)
    for i = scanFrom, COORD.lastPart do
        if #idle == 0 then break end
        if not inflight[i] and not partPath(COORD.movie, i) then
...
local function play(NAME)
    ...
    -- become the coordinator for this movie: announce it and assign parts
    COORD.active = true
    COORD.movie = NAME
    COORD.lastPart = lastPart
    tellFeader({ cmd = "play", movie = NAME })
    ...
    local function writeHead()
        ...
        local now = os.clock()
        if now - lastHeadTx > 2 then
            lastHeadTx = now
            tellFeader({ cmd = "head", n = curPart })
        end
    end
...
if argName and #argName > 0 then
    tellFeader({ cmd = "play", movie = argName })
    play(argName)
    resetPalette()
    applyTheme()
    term.clear()
...
while true do
    ...
    if it.kind == "settings" then
        ...
        markSeen(it.label)
        blinds()
        tellFeader({ cmd = "play", movie = it.label })
        play(it.label)
        resetPalette()
        applyTheme()
